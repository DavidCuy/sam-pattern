...
```bash
isiy-sam-api$ sam build --use-container
```
...
```bash
isiy-sam-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
isiy-sam-api$ sam local start-api
isiy-sam-api$ curl http://localhost:3000/
```
...
```bash
isiy-sam-api$ sam logs -n HelloWorldFunction --stack-name isiy-sam-api --tail
```
...
```bash
isiy-sam-api$ pip install -r tests/requirements.txt --user
# unit test
isiy-sam-api$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
isiy-sam-api$ AWS_SAM_STACK_NAME="isiy-sam-api" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "isiy-sam-api"
```
