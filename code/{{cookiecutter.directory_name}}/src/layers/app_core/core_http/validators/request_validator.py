...
from core_db.DBConnection import get_session
from ..utils import get_body, get_path_parameters, get_query_parameters
from sqlalchemy.sql.elements import literal
from sqlalchemy.sql.schema import Column

from ..exceptions.api_exception import APIException
from ..enums.request_parts import RequestPart
...
class RequestValidator():
    ...
    def validate(self, request: dict) -> bool:
        ...
        try:
            if self.req_part == RequestPart.BODY.value:
                self.request = get_body(request)
            if self.req_part == RequestPart.PARAM.value:
                self.request = get_path_parameters(request)
            if self.req_part == RequestPart.QUERY.value:
                self.request = get_query_parameters(request)
        except ValueError:
            self.is_valid = False
    ...
    def is_mail(self, text: str) -> Match | None:
    ...
    def exists(self, database_params: DBValidator, value: Any) -> bool:
        ...
        """
        return get_session().query(literal(True)).filter(database_params.column == value).first() is not None
